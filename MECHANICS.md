...
| 732 | Action | §732 | DONE | `loop.py:_run_showdown`, `legality._showdown_actions` | showdown-play permission for spells AND units (§732.1.c.1) — an ACTION unit can be played during a showdown (to base, via `_apply_action`). `test_action_unit.py`. (No current deck has an ACTION unit; the rule is now honored regardless.) |
...
| 743 | Vision | §743 | DONE | `predict` handler n==1 (`effects.py`) | `test_vision.py` | "look at top 1, MAY recycle". The predict handler now fires the reveal-from-top offer at n==1 and offers an optional recycle-to-bottom via `Agent.decide_predict_recycle` (default keep — a valid null action; a learning agent can recycle a bad top). |
...
| 745 | Quick-Draw | §745 | DONE | `loop.py:1442` (attach) + reaction/showdown dispatch | `test_gear_play.py`, `test_quickdraw_reaction.py` | §745.1.d attach-on-play gated on the keyword AND §745.1.a Reaction timing: a QUICK-DRAW gear is now offered/played during a reaction window and during a showdown (legality `_reaction_actions`/`_showdown_actions` + the two play loops dispatch GEAR through `_apply_action`). |
...
| Modal "choose one" | — | DONE | `mode_choice` effect + `Agent.decide_mode` | the actor picks one mode (default first); its sub-effect(s) resolve. Modes may hold multiple effects. `test_modal.py`; Minah Swiftfoot authored. The other ~8 modal cards can now be authored with `mode_choice` (corpus/parser work). |
