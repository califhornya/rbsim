...
| 745 | Quick-Draw | §745 | PARTIAL | `loop.py:1453` | — | §745.1.d = Reaction + attach-on-play. **Attach-on-play already happens** (the gear play path auto-attaches to a friendly unit at the target lane) — but for ALL gear, not just Quick-Draw, and the Reaction-speed timing is not modeled. Meta gear affected (Long Sword, Sterak's Gage, Cloth Armor). Deferred: gate auto-attach on Quick-Draw + allow reaction-window gear play. Also revisit whether normal gear should play to base (§146.1.a.1) rather than auto-attach. |
