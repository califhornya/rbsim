...
| 744 | Equip | §744 | DONE | `loop.py:1442` (play→base) + `loop.py:1739` (equip) | `test_gear_play.py`, `test_equip_deflect.py` | **Fixed:** gear now plays to BASE unattached (§146.1.a.1) instead of auto-attaching for free; it attaches via the separate Equip action. Attached gear grants might via `grant_might` (+3 verified). Caveat: the Equip action charges `cost_energy` (the play cost) as a stand-in for the true EQUIP-ability cost — a documented approximation to refine. |
