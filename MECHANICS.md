...
| Attachments / gear | §716 | DONE | `loop.py` play→base / equip / gear abilities | play→base (§146.1.a.1), Equip charges the real EQUIP cost, Quick-Draw attaches on play, attached gear grants might. **Gear's own [tap] abilities now work from base** (`gear_ability` entries + `Card.tapped` state, untapped at turn start) — 26 gear cards were previously inert. `test_gear_play.py`, `test_equip_cost.py`, `test_gear_ability.py`. "This enters exhausted" gear now enters tapped (can't tap the turn it's played). Remaining (tracked in DEFERRED.md, all required): Quick-Draw reaction-timing, Weaponmaster [A] discount, complex Equip extra-costs (recycle/kill/XP), attached-gear tap abilities. |
