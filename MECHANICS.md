...
| 733 | Assault | §733 | PARTIAL | `combat.py:171` | `test_effects.py` (keyword only) | +X might while attacker; bare = 1 (§733.1.b.3). `keyword_value` tested, but the **combat might-bonus application is not directly asserted** — add a combat test. |
...
| 740 | Shield | §740 | PARTIAL | `combat.py:171` | `test_effects.py` (keyword only) | +X might while defender; bare = 1. Combat might-bonus application not directly asserted — same combat test as Assault. |
| 741 | Tank | §741 | PARTIAL | `combat.py:96` | `test_effects.py` (keyword only) | must be assigned lethal first (§460.2.c). **Damage-assignment order not directly asserted.** Tank+Backline conflict resolved deterministically as Tank — documented simplification; confirm acceptable vs the "controller chooses" rule. |
